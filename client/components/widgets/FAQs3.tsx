  ...
  >
    <div className="flex items-stretch justify-center">
      <div className="grid w-full md:grid-cols-3 md:items-center md:gap-4">
        <div
          className={`h-full ${
            callToAction ? "block sm:flex md:block" : ""
          } sm:items-center sm:justify-between md:mt-10`}
        >
          {header && (
            <Headline
              header={header}
              titleClass={`text-3xl sm:text-4xl text-center ${
                callToAction ? "sm:text-left" : "md:text-left"
              }`}
        ...
        <div className="mt-4 h-fit md:col-span-2 md:mx-4 md:mt-0 md:px-4">
          <Collapse
            ...
            iconUp={
              <Minus className="h-6 w-6 text-primary-600 dark:text-slate-200" />
            }
            iconDown={
              <Plus className="h-6 w-6 text-primary-600 dark:text-slate-200" />
            }
          />
        </div>
      </div>
