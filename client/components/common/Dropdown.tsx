    ...
    <div className="relative mt-4 rounded-md border border-gray-400 text-left">
      ...
      >
        ...
        {iconDown && iconUp ? (
          ...
          <ChevronDown className="h-6 w-6 text-primary-600 dark:text-slate-200" />
        ) : (
          <ChevronUp className="h-6 w-6 text-primary-600 dark:text-slate-200" />
        )}
      </div>
      {isDropdownOpen && (
        <div className="absolute w-full translate-y-1 overflow-auto rounded-md border border-gray-400">
          {options.map((option: Tab, index) => (
            ...
            >
              {activeTab === index && <Check className="mr-2 h-5 w-5" />}{" "}
              {option.link?.label}
            </div>
          ))}
        </div>
